...
score_case <- function(significance_sql) {
  sql(
    ...
    sprintf("  WHEN %s = 'conflicting' THEN 1", significance_sql),
    "  ELSE 1",
    "END"
  )
}
...
load_clinvar <- function(path, build_label, row_limit = 0L) {
  ...
  clinvar_sql <- sql(
    ...
    "  CASE",
    "    WHEN contains(lower(coalesce(clnsig_text, '')), 'conflicting')",
    "    THEN 'uncertain'",
    "    ELSE 'health_risk'",
...
load_clinvar_tsv <- function(path, row_limit = 0L) {
  ...
  clinvar_tsv_sql <- sql(
    ...
    "    AND NULLIF(ReferenceAlleleVCF, 'na') IS NOT NULL",
    "    AND NULLIF(AlternateAlleleVCF, 'na') IS NOT NULL",
    "    AND ReferenceAlleleVCF <> AlternateAlleleVCF",
    "    AND (",
    "      lower(coalesce(ClinicalSignificance, '')) LIKE '%pathogenic%'",
    ...
    "  'clinvar'::VARCHAR AS source,",
    "  CASE",
    "    WHEN coalesce(\"RS# (dbSNP)\", '') NOT IN ('', '-', '-1', '0')",
    "    THEN 'rs' || \"RS# (dbSNP)\"",
    ...
    "  CASE",
    "    WHEN clinsig_lc LIKE '%conflicting%' THEN 'uncertain'",
    ...
    "  CASE",
    "    WHEN clinsig_lc LIKE '%conflicting%' THEN 'conflicting'",
    ...
    "  (",
    "    CASE",
    "      WHEN clinsig_lc LIKE '%conflicting%' THEN 1",
...
manifest <- list(
  ...
  schema_version = 3,
  matching_policy = "locus_first_with_vcf_variantkey_refinement",
  ...
  notes = c(
    paste(
      ...
      "and 1-based position; VCF/BCF runtime joins also require exact",
      "variant_key allele agreement when annotation REF/ALT keys exist."
    ),
